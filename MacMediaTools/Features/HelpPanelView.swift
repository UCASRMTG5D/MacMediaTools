...
private let helpFeatures: [HelpFeature] = [
	HelpFeature(
		...
		details: [
			...
			"裁剪：拖拽黄色裁剪框（拖动边框移动位置、拖动顶点自由调整大小，裁剪框不会超出图片范围），支持 1:1 / 4:3 / 16:9 等宽高比预设，数字输入宽高双向同步，正确处理旋转元数据",
			"尺寸调整：输入目标宽高，支持一键套用 1:1 / 4:3 / 16:9 等宽高比预设与恢复原始尺寸，支持拉伸 / Aspect Fit（保持比例加黑边）",
